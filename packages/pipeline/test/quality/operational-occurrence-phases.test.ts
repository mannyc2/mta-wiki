...
describe("operational occurrence phase review v1", () => {
  ...
  it("pins the complete 135-row corpus review and every generated byte", () => {
    ...
    const manifest = JSON.parse(readFileSync(join(qualityDir, "manifest.json"), "utf8")) as {
      ...
      route_anchor_release: { release_id: string; manifest: { path: string } };
    ...
    expect(summary).toMatchObject({
      ...
      reviewed_occurrence_count: 135,
      single_observed_phase_count: 135,
      related_phase_count: 0,
    ...
    expect(ledger).toHaveLength(135);
    expect(new Set(ledger.map((row) => row.occurrence_id)).size).toBe(135);
    ...
    expect(manifest).toMatchObject({
      ...
      route_anchor_release: {
        release_id: "v1-rc21",
        manifest: { path: "data/exports/releases/v1-rc21/manifest.json" },
      },
    });
