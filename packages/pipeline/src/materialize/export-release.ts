...
function manifestPointer(value: unknown, path: string): string | null {
  ...
  return manifestString(value, path);
}
...
export function parseReleaseManifest(value: unknown): ReleaseManifest {
  ...
  if (version !== 1 && version !== 2) throw new Error("Invalid release manifest manifest_version: expected 1 or 2");
  assertManifestKeys(
    root,
    version === 1
  ...
  const operationalAnchors = version === 2 ? manifestPointer(pointersInput.operational_anchors, "pointers.operational_anchors") : null;
  const operationalAnchorSummary =
    version === 2 ? manifestPointer(pointersInput.operational_anchor_summary, "pointers.operational_anchor_summary") : null;
  ...
  if (version === 2) {
    ...
    assertManifestKeys(input, ["operational_anchors", "operational_anchor_review_decisions"], "contract_versions");
