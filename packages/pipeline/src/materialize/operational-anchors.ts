...
export function operationalAnchorsJsonl(rows: readonly OperationalAnchorRow[]): string {
  ...
  parseOperationalAnchorsJsonl(json);
  return json;
}

const operationalAnchorRowFields = ["schema_version","anchor_id","operational_change_id","event_record_id","timeline_relation_record_ids","project_record_ids","subject_record_ids","subject_record_kinds","route_record_ids","unmatched_route_record_ids","gtfs_route_ids","treatment_record_ids","treatment_families","route_scope_direct","treatment_scope_direct","temporal_role","raw_date","normalized_date","date_precision","candidate_operational_date_raw","candidate_operational_date_normalized","candidate_operational_date_precision","candidate_operational_date_source_field","candidate_operational_date_candidates","candidate_operational_dates_normalized","status_as_of_dates","event_family","lifecycle_phase","assertion_statuses","truth_status","truth_statuses","review_state","source_id","source_ids","source_authority","source_publishers","route_scope_resolution","treatment_scope_resolution","scope_resolution","conflict_states","evidence_coverage","evidence_refs","exclusion_reasons","study_eligible"] as const;
function anchorObject(value: unknown, path: string): Record<string, unknown> { if (typeof value !== "object" || value === null || Array.isArray(value)) throw new Error(`${path}: expected object`); return value as Record<string, unknown>; }
function anchorKeys(value: Record<string, unknown>, fields: readonly string[], path: string): void { const extras = Object.keys(value).filter((key) => !fields.includes(key)); const missing = fields.filter((key) => !(key in value)); if (extras.length || missing.length) throw new Error(`${path}: keys must be exactly ${fields.join(", ")}`); }
function anchorString(value: unknown, path: string): string { if (typeof value !== "string" || !value.trim()) throw new Error(`${path}: expected non-empty string`); return value; }
function anchorStrings(value: unknown, path: string): string[] { if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item.trim())) throw new Error(`${path}: expected string array`); return value as string[]; }
function anchorNullableString(value: unknown, path: string): string | null { if (value === null) return null; return anchorString(value, path); }
function anchorEnum<T extends string>(value: unknown, allowed: readonly T[], path: string): T { if (typeof value !== "string" || !allowed.includes(value as T)) throw new Error(`${path}: unsupported value ${String(value)}`); return value as T; }
export function parseOperationalAnchor(value: unknown, path = "operational anchor"): OperationalAnchorRow {
  const row = anchorObject(value, path); anchorKeys(row, operationalAnchorRowFields, path);
  if (row.schema_version !== 1) throw new Error(`${path}.schema_version: expected 1`);
  const stringFields = ["anchor_id","operational_change_id","event_record_id","date_precision","candidate_operational_date_precision","event_family","truth_status","review_state","source_id"] as const; for (const field of stringFields) anchorString(row[field], `${path}.${field}`);
  const arrayFields = ["timeline_relation_record_ids","project_record_ids","subject_record_ids","subject_record_kinds","route_record_ids","unmatched_route_record_ids","gtfs_route_ids","treatment_record_ids","treatment_families","candidate_operational_dates_normalized","status_as_of_dates","assertion_statuses","truth_statuses","source_ids","source_publishers"] as const; for (const field of arrayFields) anchorStrings(row[field], `${path}.${field}`);
  for (const field of ["route_scope_direct","treatment_scope_direct","study_eligible"] as const) if (typeof row[field] !== "boolean") throw new Error(`${path}.${field}: expected boolean`);
  for (const field of ["raw_date","normalized_date","candidate_operational_date_raw","candidate_operational_date_normalized","candidate_operational_date_source_field","lifecycle_phase"] as const) anchorNullableString(row[field], `${path}.${field}`);
  anchorEnum(row.temporal_role, ["status_as_of","planned_operational","realized_operational"], `${path}.temporal_role`);
  for (const field of ["route_scope_resolution","treatment_scope_resolution","scope_resolution"] as const) anchorEnum(row[field], ["direct","reviewed_inherited","unreviewed_inherited","ambiguous","missing"], `${path}.${field}`);
  anchorEnum(row.source_authority, ["mixed","non_official","official_public_agency","unknown"], `${path}.source_authority`);
  if (!Array.isArray(row.candidate_operational_date_candidates)) throw new Error(`${path}.candidate_operational_date_candidates: expected array`); row.candidate_operational_date_candidates.forEach((entry, index) => { const item = anchorObject(entry, `${path}.candidate_operational_date_candidates[${index}]`); anchorKeys(item, ["source_field","raw","normalized","precision","origin"], `${path}.candidate_operational_date_candidates[${index}]`); for (const field of ["source_field","raw","normalized","precision"] as const) anchorString(item[field], `${path}.candidate_operational_date_candidates[${index}].${field}`); anchorEnum(item.origin, ["canonical_scalar","merged_field","normalized_companion","payload_field"], `${path}.candidate_operational_date_candidates[${index}].origin`); });
  const coverage = anchorObject(row.evidence_coverage, `${path}.evidence_coverage`); anchorKeys(coverage, ["event","timeline","route_scope","treatment_scope"], `${path}.evidence_coverage`); for (const field of ["event","timeline","route_scope","treatment_scope"] as const) if (typeof coverage[field] !== "boolean") throw new Error(`${path}.evidence_coverage.${field}: expected boolean`);
  if (!Array.isArray(row.evidence_refs)) throw new Error(`${path}.evidence_refs: expected array`); row.evidence_refs.forEach((entry, index) => { const item = anchorObject(entry, `${path}.evidence_refs[${index}]`); anchorKeys(item, ["record_id","source_id","evidence_id","block_id","page_number","text_sha256","role"], `${path}.evidence_refs[${index}]`); anchorString(item.record_id, `${path}.evidence_refs[${index}].record_id`); anchorString(item.source_id, `${path}.evidence_refs[${index}].source_id`); for (const field of ["evidence_id","block_id","text_sha256"] as const) anchorNullableString(item[field], `${path}.evidence_refs[${index}].${field}`); if (item.page_number !== null && (!Number.isInteger(item.page_number) || (item.page_number as number) < 1)) throw new Error(`${path}.evidence_refs[${index}].page_number: expected positive integer or null`); anchorEnum(item.role, ["event","route_scope","timeline_relation","treatment_scope"], `${path}.evidence_refs[${index}].role`); });
  if (!Array.isArray(row.conflict_states)) throw new Error(`${path}.conflict_states: expected array`); row.conflict_states.forEach((item, index) => anchorEnum(item, ["date_conflict","route_identity_conflict","status_conflict","temporal_order_conflict"], `${path}.conflict_states[${index}]`));
  if (!Array.isArray(row.exclusion_reasons)) throw new Error(`${path}.exclusion_reasons: expected array`); const exclusionReasons = ["ambiguous_lifecycle_phase","ambiguous_route_scope","ambiguous_treatment_scope","conflicting_date_evidence","conflicting_route_identity","conflicting_status_evidence","imprecise_operational_date","missing_event_evidence","missing_operational_date","missing_route_scope","missing_route_scope_evidence","missing_timeline_evidence","missing_treatment_scope","missing_treatment_scope_evidence","missing_treatment_family","non_realized_operational_date","non_source_stated_evidence","partially_unmatched_gtfs_route","quarantined_record","status_as_of_only","future_delivered_status","unconfirmed_inherited_scope","unmatched_gtfs_route","unreviewed_inherited_scope","untrusted_source_authority","unsupported_subject_scope"] as const; row.exclusion_reasons.forEach((item, index) => anchorEnum(item, exclusionReasons, `${path}.exclusion_reasons[${index}]`));
  return row as unknown as OperationalAnchorRow;
}
export function parseOperationalAnchorsJsonl(value: string): OperationalAnchorRow[] { return value.split(/\r?\n/u).filter(Boolean).map((line, index) => { try { return parseOperationalAnchor(JSON.parse(line) as unknown, `operational anchors line ${index + 1}`); } catch (error) { throw new Error(`operational anchors line ${index + 1}: ${error instanceof Error ? error.message : String(error)}`); } }); }
...
export function operationalAnchorSummaryJson(summary: OperationalAnchorSummary): string {
  ...
  parseOperationalAnchorSummary(JSON.parse(json) as unknown);
  return json;
}

const summaryFields = ["schema_version","row_count","broad_row_count","reviewed_row_count","distinct_operational_event_count","study_eligible_count","study_eligible_reviewed_count","counts_by_temporal_role","counts_by_scope_resolution","counts_by_exclusion_reason","entry_gate","broad_funnel","funnel"] as const;
const entryGateFields = ["relations_examined","non_event_timeline_objects","non_operational_event_objects"] as const;
const broadFunnelFields = ["operational_family_events_total","timeline_linked_distinct_events","unlinked_operational_events","candidate_operational_date_present","realized_operational","realized_day_or_month","resolved_route_scope","resolved_treatment_scope","evidence_complete","conflict_free","study_eligible"] as const;
const funnelFields = ["canonical_events","operational_family_events_total","timeline_linked_operational_events","timeline_linked_distinct_events","unlinked_operational_events","candidate_operational_date_present","realized_operational","realized_day_or_month","resolved_route_scope","resolved_treatment_scope","evidence_complete","conflict_free","study_eligible"] as const;
function summaryIntegers(value: unknown, fields: readonly string[], path: string): Record<string, unknown> { const object = anchorObject(value, path); anchorKeys(object, fields, path); for (const field of fields) if (!Number.isSafeInteger(object[field]) || (object[field] as number) < 0) throw new Error(`${path}.${field}: expected non-negative integer`); return object; }
function summaryCounts(value: unknown, path: string): Record<string, number> { const object = anchorObject(value, path); for (const [key, count] of Object.entries(object)) if (!Number.isSafeInteger(count) || (count as number) < 0) throw new Error(`${path}.${key}: expected non-negative integer`); return object as Record<string, number>; }
export function parseOperationalAnchorSummary(value: unknown, path = "operational_anchors_summary.json"): OperationalAnchorSummary {
  const root = anchorObject(value, path); anchorKeys(root, summaryFields, path);
  if (root.schema_version !== OPERATIONAL_ANCHOR_SCHEMA_VERSION) throw new Error(`${path}.schema_version: expected ${OPERATIONAL_ANCHOR_SCHEMA_VERSION}`);
  for (const field of ["row_count","broad_row_count","reviewed_row_count","distinct_operational_event_count","study_eligible_count","study_eligible_reviewed_count"] as const) if (!Number.isSafeInteger(root[field]) || (root[field] as number) < 0) throw new Error(`${path}.${field}: expected non-negative integer`);
  for (const field of ["counts_by_temporal_role","counts_by_scope_resolution","counts_by_exclusion_reason"] as const) summaryCounts(root[field], `${path}.${field}`);
  summaryIntegers(root.entry_gate, entryGateFields, `${path}.entry_gate`); summaryIntegers(root.broad_funnel, broadFunnelFields, `${path}.broad_funnel`); summaryIntegers(root.funnel, funnelFields, `${path}.funnel`);
  return root as unknown as OperationalAnchorSummary;
}
