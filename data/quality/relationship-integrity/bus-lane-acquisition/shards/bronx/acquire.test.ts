...
describe("Bronx registry-only bus-lane acquisition", () => {
  test("reconciles exactly to the pinned 52-row Bronx partition", () => {
    ...
    expect(proof.bronx_count).toBe(52);
    expect(proof.bx_local_or_sbs_count).toBe(41);
    expect(proof.bxm_express_count).toBe(11);
    expect(partition).toHaveLength(52);
    expect(new Set(partition.map((row) => row.candidate_id)).size).toBe(52);
    expect(new Set(partition.map((row) => row.route_id)).size).toBe(52);
    expect(partition.filter((row) => /^BX\d/.test(row.route_id))).toHaveLength(41);
    expect(partition.filter((row) => /^BXM\d/.test(row.route_id))).toHaveLength(11);
  ...
  test("records four genuine official acquisition channels for every candidate", () => {
    ...
    expect(checks.sources).toHaveLength(23);
    expect(checks.route_pages).toHaveLength(52);
    expect(
  ...
  test("supports only explicit route-project statements and never promotes proximity", () => {
    const receipts = readJsonl<{
      ...
      source_findings: { candidate_named_lane_record_count: number; exact_project_route_statement_found: boolean };
      claim_results: {
    ...
    expect(routeBound).toHaveLength(13);
    ...
    expect(routeBound.every((receipt) => receipt.source_findings.candidate_named_lane_record_count === 0)).toBe(true);
    expect(
      routeBound.every(
        (receipt) =>
    ...
    expect(routeBound.some((receipt) => /^BX18/.test(receipt.candidate.route_id))).toBe(false);

    const bx12Local = receipts.find(
      (receipt) => receipt.candidate.candidate_id === "study-event-v2:4f20a93956a3af9db4bad8c1",
    );
    expect(bx12Local?.candidate.route_id).toBe("BX12");
    expect(bx12Local?.source_findings.exact_project_route_statement_found).toBe(false);
    expect(bx12Local?.claim_results.exact_route_treatment_binding_proved).toBe(false);
    expect(supported.some((row) => row.candidate_id === bx12Local?.candidate.candidate_id)).toBe(false);
  });
  ...
  test("reconciles the receipt and exclusion outcomes exactly", () => {
    ...
    expect(new Set(exclusions.map((row) => row.candidate_id)).size).toBe(52);
    expect(exclusions.every((row) => !row.study_projection_eligible)).toBe(true);
    expect(summary.researched_count).toBe(52);
    expect(summary.source_acquired_count).toBe(52);
    expect(summary.exact_route_binding_proved_count).toBe(13);
    expect(summary.route_variant_precision_rejected_count).toBe(1);
    ...
    expect(summary.explicitly_excluded_count).toBe(52);
    expect(summary.still_unresolved_count).toBe(52);
    expect(summary.study_projection_eligible_count).toBe(0);
    const bx12Local = exclusions.find(
      (row) => row.candidate_id === "study-event-v2:4f20a93956a3af9db4bad8c1",
    );
    expect(bx12Local?.exact_route_treatment_binding_proved).toBe(false);
