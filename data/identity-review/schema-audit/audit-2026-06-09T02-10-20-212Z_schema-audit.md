...
run_id: 2026-06-09T02-10-20-212Z_schema-audit
generated_at: 2026-06-09T02:10:20.212Z

Corpus: 3360 submissions (accepted 3232 / rejected 128) across 12 observation kinds.
Canonical records in audit projection: 2753.
...
| unit | — | 949 | 931 | 936 | 0.85 | scalar_string | 95 | free_text | percent, minutes, mph, miles, dollars, vehicles, intersections, routes |
...
| unit_normalized | — | 221 | 220 | 936 | 0.20 | object | 0 | structured |  |
...
| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| relation_kind | yes | 495 | 451 | 450 | 1.00 | scalar_string | 115 | free_text | has_timeline_event, serves_route, in_development_for, has_treatment,… |
| subject_local_observation_id | yes | 495 | 451 | 450 | 1.00 | scalar_string | 148 | free_text | project_ace_automated_camera_enforcement, project_tsp_expansion_2017,… |
| object_local_observation_id | yes | 491 | 451 | 450 | 0.99 | scalar_string | 391 | free_text | entity_nyc_dot, entity_mta_nyct, entity_nyc-dot, project_open_data_pl… |
| description | — | 222 | 187 | 187 | 0.45 | scalar_string | 189 | free_text | B82 in Southern Brooklyn in development for TSP, Bx12 SBS on Fordham… |
...
| object_id | — | 0 | 0 | 450 | 0.00 | empty | 0 | sparse |  |
| subject_id | — | 0 | 0 | 450 | 0.00 | empty | 0 | sparse |  |
...
| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| claim_text | yes | 397 | 382 | 380 | 0.93 | scalar_string | 383 | free_text | A 2025 report assessing corridor-level speed impacts of different bus… |
...
| route_name | yes | 109 | 106 | 68 | 0.42 | scalar_string | 100 | free_text | M86, Q43, B82, Bx6, M15, Q25, Q52 SBS, 125th-LaGuardia Airport Select… |
| borough | — | 106 | 104 | 78 | 0.41 | mixed | 5 | enum_candidate | Bronx, Manhattan, Brooklyn, Queens, Staten Island |
| description | — | 92 | 91 | 69 | 0.36 | scalar_string | 75 | free_text | Bronx-Manhattan express bus route serving 116th Street study area, Br… |
| route | yes | 66 | 66 | 39 | 0.26 | scalar_string | 56 | free_text | B25, B26, B42, B62, BX19, BX35, BX36, Q43 |
| route_type | — | 61 | 58 | 44 | 0.24 | scalar_string | 14 | free_text | Select Bus Service, Local, local bus, SBS, express_bus, select_bus_se… |
| streets | — | 61 | 61 | 61 | 0.24 | scalar_string | 61 | free_text | 1 Av / 2 Av, 116 St / Manhattan Av, 125 St / Astoria Blvd, 14 St, 21… |
| route_type_normalized | — | 43 | 40 | 44 | 0.17 | scalar_string | 6 | enum_candidate | local, select_bus_service, bus, limited_stop, local_limited, local_se… |
| service_variant | — | 38 | 36 | 43 | 0.15 | scalar_string | 3 | enum_candidate | local, sbs, limited_stop |
| borough_normalized | — | 28 | 26 | 78 | 0.11 | scalar_string | 5 | enum_candidate | bronx, manhattan, queens, brooklyn, staten_island |
| note | — | 26 | 26 | 26 | 0.10 | scalar_string | 3 | free_text | ABLE cameras operated on this route through 2023, in 60-day warning p… |
| program | — | 21 | 21 | 21 | 0.08 | scalar_string | 1 | enum_candidate | ABLE |
...
| _merged_field_values | — | 0 | 0 | 32 | 0.00 | empty | 0 | sparse |  |
...
- **borough** (106 all / 104 accepted / 78 canonical, 5 distinct, singletons 0/5, **saturated → closure candidate**): `Bronx` | `Brooklyn` | `Manhattan` | `Queens` | `Staten Island`
  - counts: Bronx×28, Manhattan×27, Brooklyn×24, Queens×20, Staten Island×8
  - accepted counts: Bronx×27, Manhattan×27, Brooklyn×23, Queens×20, Staten Island×8
  - canonical counts: Bronx×21, Manhattan×20, Brooklyn×16, Queens×16, Staten Island×6
- **route_type_normalized** (43 all / 40 accepted / 44 canonical, 6 distinct, singletons 2/6, open): `bus` | `limited_stop` | `local` | `local_limited` | `local_select_bus_service` | `select_bus_service`
  - counts: local×18, select_bus_service×18, bus×3, limited_stop×2
  - accepted counts: select_bus_service×18, local×16, bus×3, limited_stop×2
  - canonical counts: local×17, select_bus_service×13, express×6, limited_stop×3, bus×2, rush×2
- **service_variant** (38 all / 36 accepted / 43 canonical, 3 distinct, singletons 0/3, open): `limited_stop` | `local` | `sbs`
  - counts: local×18, sbs×18, limited_stop×2
  - accepted counts: sbs×18, local×16, limited_stop×2
  - canonical counts: local×17, sbs×15, express×6, limited_stop×3, rush×2
- **borough_normalized** (28 all / 26 accepted / 78 canonical, 5 distinct, singletons 0/5, open): `bronx` | `brooklyn` | `manhattan` | `queens` | `staten_island`
  - counts: bronx×8, manhattan×7, queens×5, brooklyn×4, staten_island×4
  - accepted counts: bronx×7, manhattan×7, queens×5, staten_island×4, brooklyn×3
  - canonical counts: bronx×20, manhattan×20, brooklyn×16, queens×16, staten_island×6
...
submissions: 239 (accepted 235 / rejected 4); canonical records: 98

### Fields

| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| entity_name | yes | 232 | 228 | 98 | 0.97 | scalar_string | 122 | free_text | New York City Department of Transportation, MTA New York City Transit… |
| entity_type | yes | 221 | 217 | 94 | 0.92 | scalar_string | 77 | free_text | person, government_agency, transit_agency, government agency, agency,… |
| agency_name | yes | 91 | 88 | 21 | 0.38 | scalar_string | 24 | free_text | NYC DOT, MTA, Metropolitan Transportation Authority, MTA New York Cit… |
| description | — | 57 | 56 | 43 | 0.24 | scalar_string | 56 | free_text | Automated Camera Enforcement program for bus lane enforcement, Advoca… |
...
| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| description | yes | 219 | 211 | 209 | 1.00 | scalar_string | 202 | free_text | Developed / Published Plan, Busway launched October 2021, Capital imp… |
| event_kind | yes | 219 | 211 | 209 | 1.00 | scalar_string | 99 | free_text | service_launch, publication, launch, milestone, implementation, meeti… |
| date_text | yes | 202 | 196 | 194 | 0.92 | scalar_string | 151 | free_text | 2024, 2023, 2019, October 3, 2019, 2025, October 2019, October 2021,… |
| date_text_normalized | — | 202 | 196 | 194 | 0.92 | object | 0 | structured |  |
...
| event_family | — | 12 | 11 | 209 | 0.05 | scalar_string | 8 | enum_candidate | public_engagement, milestone, construction, launch, press_release, pu… |
...
- **event_family** (12 all / 11 accepted / 209 canonical, 8 distinct, singletons 6/8, open): `construction` | `launch` | `milestone` | `press_release` | `public_engagement` | `publication` | `subway_service_start` | `tolling_program_commencement`
  - counts: public_engagement×4, milestone×2
  - accepted counts: public_engagement×4
  - canonical counts: launch×41, public_engagement×32, milestone×23, implementation×22, publication×18, construction×8, data_release×7, enforcement×6, legislation×6, pause×4, blog_post×3, approval×2, datathon×2, page_update×2
...
- **treatment_family** (18 all / 18 accepted / 163 canonical, 11 distinct, singletons 6/11, open): `bus_lane` | `bus_stop_or_boarding` | `busway` | `curb_management` | `enforcement` | `fare_collection` | `pedestrian_or_accessibility` | `shelters_and_benches` | `signage_and_markings` | `signal_priority` | `traffic_restriction`
  ...
  - canonical counts: traffic_restriction×28, bus_lane×19, curb_management×14, bus_stop_or_boarding×12, enforcement×12, pedestrian_or_accessibility×11, busway×10, bus_priority×7, signal_priority×7, service_pattern×6, signage_and_markings×6, fare_collection×4, bike_facility×2, bus_infrastructure_conversion×2, shelters_and_benches×2
...
| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| project_name | yes | 138 | 133 | 105 | 1.00 | scalar_string | 110 | free_text | Queens Bus Network Redesign, 14th Street Transit & Truck Priority Pil… |
| status | yes | 132 | 127 | 103 | 0.96 | scalar_string | 43 | free_text | proposed_2019, completed, active, proposed, study, ongoing, operation… |
| description | yes | 131 | 126 | 102 | 0.95 | scalar_string | 126 | free_text | A bus-mounted camera system that issues violations to vehicles occupy… |
| project_type | yes | 115 | 110 | 87 | 0.83 | scalar_string | 67 | free_text | busway, new_bus_lane, bus_network_redesign, bus network redesign, bus… |
| borough | — | 40 | 39 | 36 | 0.29 | scalar_string | 5 | enum_candidate | Manhattan, Brooklyn, Bronx, Queens, Staten Island |
| document_time_status | — | 25 | 23 | 103 | 0.18 | scalar_string | 8 | enum_candidate | implemented, active, stalled_resuming, announced, study, construction… |
...
| project_family | — | 24 | 22 | 87 | 0.17 | scalar_string | 15 | free_text | busway, capital_or_infrastructure, bike_lane, bus_lane, bus_network_r… |
...
| location | — | 5 | 5 | 5 | 0.04 | scalar_string | 5 | free_text | 14th Street, Manhattan, Bronx, New York City, Church Avenue Station,… |
| location_normalized | — | 5 | 5 | 5 | 0.04 | object | 0 | structured |  |
| operator | — | 5 | 5 | 5 | 0.04 | scalar_string | 2 | enum_candidate | NYC DOT, MTA |
...
| _merged_field_values | — | 0 | 0 | 13 | 0.00 | empty | 0 | sparse |  |
...
- **document_time_status** (25 all / 23 accepted / 103 canonical, 8 distinct, singletons 3/8, open): `active` | `announced` | `construction_began_june_2016_anticipated_completion_summer_2017` | `implemented` | `launched_july_2015_post_implementation_progress_reported` | `planned` | `stalled_resuming` | `study`
  - counts: implemented×11, active×4, stalled_resuming×3, announced×2, study×2
  - accepted counts: implemented×11, active×3, stalled_resuming×3, study×2
  - canonical counts: planned×40, implemented×27, active×13, study×10, stalled_resuming×3, under_construction×3, approved×2, pilot×2
...
| field | anchor | all | acc | canon | coverage | value_kind | distinct | classification | samples |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| corridor_name | yes | 86 | 83 | 54 | 1.00 | scalar_string | 63 | free_text | Tremont Avenue, 34th Street Busway, Jamaica Avenue, Story Avenue, 116… |
| borough | — | 57 | 54 | 41 | 0.66 | scalar_string | 6 | enum_candidate | Manhattan, Brooklyn, Queens, Bronx, Staten Island, the Bronx |
| limits | yes | 51 | 49 | 35 | 0.59 | scalar_string | 47 | free_text | 3rd Avenue to 8th Avenue (westbound) and 9th Avenue to 3rd Avenue (ea… |
| street | yes | 50 | 48 | 32 | 0.58 | scalar_string | 33 | free_text | 14th Street, Tremont Avenue, Jamaica Avenue, 34th Street, Archer Aven… |
| description | yes | 47 | 46 | 40 | 0.55 | scalar_string | 46 | free_text | Bus priority corridor in Soundview, South Bronx with new curbside bus… |
...
| borough_normalized | — | 12 | 11 | 41 | 0.14 | scalar_string | 4 | enum_candidate | bronx, manhattan, brooklyn, queens |
| status | — | 12 | 12 | 11 | 0.14 | scalar_string | 4 | enum_candidate | Completed 2020, Future Plan, Planned, Present Implementation |
| corridor_length_mi | — | 10 | 10 | 10 | 0.12 | number | 0 | numeric |  |
| routes | — | 10 | 10 | 10 | 0.12 | array_string | 29 | free_text | Bx36, Bx3, Bx35, Bx11, Bx12, Bx12 SBS, Bx13, Bx17 |
| routes_served | — | 10 | 10 | 10 | 0.12 | array_string | 26 | free_text | Bx35, Bx36, Bx11, Bx12, Bx12 SBS, Bx13, Bx17, Bx18 |
...
- **borough** (57 all / 54 accepted / 41 canonical, 6 distinct, singletons 1/6, open): `Bronx` | `Brooklyn` | `Manhattan` | `Queens` | `Staten Island` | `the Bronx`
  - counts: Manhattan×17, Brooklyn×13, Queens×13, Bronx×11, Staten Island×2
  - accepted counts: Manhattan×16, Brooklyn×13, Queens×12, Bronx×10, Staten Island×2
  - canonical counts: Brooklyn×13, Manhattan×10, Bronx×8, Queens×8, Staten Island×2
- **borough_normalized** (12 all / 11 accepted / 41 canonical, 4 distinct, singletons 0/4, open): `bronx` | `brooklyn` | `manhattan` | `queens`
  - counts: bronx×5, manhattan×3, brooklyn×2, queens×2
  - accepted counts: bronx×4, manhattan×3, brooklyn×2, queens×2
  - canonical counts: brooklyn×13, manhattan×10, bronx×8, queens×8, staten_island×2
