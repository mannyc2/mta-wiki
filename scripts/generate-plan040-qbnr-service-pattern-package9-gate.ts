...
import {
  ...
  writePlan040Package9GateAndAcceptance,
} from "../packages/pipeline/src/quality/plan040-qbnr-service-pattern-package9-closeout";

const ACCEPTED_AT = "2026-07-24T12:08:06Z";
const check = process.argv.includes("--check");
if (
  check &&
  (
    !existsSync(PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_9_GATE_PATH) ||
    !existsSync(PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_9_ACCEPTANCE_PATH)
  )
) {
  throw new Error(
    "Plan 040 Package 9 gate/acceptance artifacts are missing",
  );
}
const result = writePlan040Package9GateAndAcceptance({
...
if (
  ...
  throw new Error(
    "Plan 040 Package 9 gate/acceptance replay hash drifted",
  );
}
