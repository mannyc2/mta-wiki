...
function buildStudyArtifacts(input: JsonObject, occurrences: JsonObject[], extents: MemberExtentRow[]) {
  ...
  if (
    reviewedPriorityTargets.length !== 11 ||
    openPriorityTargets.length !== 3 ||
    resolvedTargets.length !== 11
  ) {
    throw new Error(
      `Expected 11 reviewed / 3 open priority / 11 replay targets, received ` +
        `${reviewedPriorityTargets.length}/${openPriorityTargets.length}/` +
