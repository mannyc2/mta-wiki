...
import {
  ...
  acceptPlan040Package10dDecisionPackage,
} from
...
if (
  check &&
  (
    !existsSync(PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_10D_EXTENT_DECISIONS_PATH) ||
    !existsSync(PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_10D_GRAIN_DECISIONS_PATH)
  )
) {
  throw new Error("Plan 040 Package 10D persistence artifacts are missing");
}
const result = acceptPlan040Package10dDecisionPackage();
if (
  ...
  throw new Error("Plan 040 Package 10D persistence replay drifted");
}
