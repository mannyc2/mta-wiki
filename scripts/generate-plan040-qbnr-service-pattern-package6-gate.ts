...
import {
  ...
  writePlan040Package6GateAndAcceptance,
} from "../packages/pipeline/src/quality/plan040-qbnr-service-pattern-package6";

const ACCEPTED_AT = "2026-07-24T07:46:30Z";
const check = process.argv.includes("--check");
if (
  check &&
  (
    !existsSync(PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_6_GATE_PATH) ||
    !existsSync(
      PLAN040_QBNR_SERVICE_PATTERN_PACKAGE_6_ACCEPTANCE_PATH,
    )
  )
  ...
  throw new Error(
    "Plan 040 Package 6 gate/acceptance artifacts are missing",
  );
}
const result = writePlan040Package6GateAndAcceptance({
...
if (
  ...
  throw new Error(
    "Plan 040 Package 6 gate/acceptance replay hash drifted",
  );
}
