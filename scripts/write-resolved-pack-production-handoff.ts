...
const RELEASE_ID = "resolved-pack-v1-production-candidate";
const AS_OF_DATE = "2026-07-27";
const ROOT_A = "/tmp/resolved-pack-production-a";
const ROOT_B = "/tmp/resolved-pack-production-b";
const LEGACY_ROOT = "/tmp/resolved-pack-release-a";
const OUTPUT = join(repoRoot, "data/quality/releases/resolved-pack-v1-production-handoff.json");
...
if (latest !== "v1-rc26" || existsSync(join(repoRoot, "data/exports/releases", RELEASE_ID))) {
  throw new Error("repository release state changed before the publication gate");
}

const core = {
  schema_version: 1,
  contract_id: "resolved-pack-v1-production-handoff-v1",
...
const output = { ...core, handoff_id: `resolved-pack-v1-production-handoff:${sha256(stableJson(core as unknown as JsonValue))}` };
